...
class VirusTotalAPI:
    ...
    def check_url(self, url):
        ...
        try:
            ...
            timeout = 45  # seconds - increased timeout for complex URLs
            ...
            while time.time() - start_time < timeout:
                ...
                if analysis_result:
                    ...
                    if status == 'completed':
                        ...
                        time.sleep(3)  # Increased wait between checks
                    else:
                        if status:
    ...
    def _get_url_report(self, url):
        ...
        if response.status_code == 200:
            try:
                ...
                if not stats:
                    logging.warning("No analysis stats in VirusTotal response")
                    return None

                logging.info(f"VirusTotal stats received: {stats}")
                return {
                ...
                logging.error(f"Error processing VirusTotal response: {e}")
                return None
        else:
